...
void writeState(nvpair* localpairs, int localpaircount, char* statename){
  ...
  nv = (nvpair){"GLOBAL:line_buf", "char[]", line_buf, sizeof(char)*LINE_COUNTER_BUF_LEN};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:line_num_print", "char *", line_num_print, sizeof(char)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:line_num_start", "char *", line_num_start, sizeof(char)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:line_num_end", "char *", line_num_end, sizeof(char)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:newlines2", "int", &newlines2, sizeof(int)};
  writeValue(nv, statefile);
