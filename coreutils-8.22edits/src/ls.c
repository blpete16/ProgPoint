...
FILE* statefile;
char* statefilename = "/home/brian/Desktop/gradschool/ls_statefile.dat";
...
void writeState(nvpair* localpairs, int localpaircount, char* statename){
  ...
  nv = (nvpair){"GLOBAL:abmon", "char[][]", abmon, sizeof(char) * 12* (MAX_MON_WIDTH * 2 * MB_LEN_MAX + 1)};
  ...
  nv = (nvpair){"GLOBAL:cwd_file", "struct fileinfo *", cwd_file, sizeof(struct fileinfo *)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:cwd_n_alloc", "size_t", &cwd_n_alloc, sizeof(size_t)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:cwd_n_used", "size_t", &cwd_n_used, sizeof(size_t)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:sorted_file", "void **", sorted_file, sizeof(void**)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:sorted_file_alloc", "size_t", &sorted_file_alloc, sizeof(size_t)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:color_symlink_as_referent", "bool", &color_symlink_as_referent, sizeof(bool)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:pending_dirs", "struct pending *", pending_dirs, sizeof(struct pending*)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:current_time", "struct timespec", &current_time, sizeof(struct timespec)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:print_scontext", "bool", &print_scontext, sizeof(bool)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:UNKNOWN_SECURITY_CONTEXT", "char[]", UNKNOWN_SECURITY_CONTEXT, sizeof(char)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:any_has_acl", "bool", &any_has_acl, sizeof(bool)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:inode_number_width", "int", &inode_number_width, sizeof(int)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:block_size_width", "int", &block_size_width, sizeof(int)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:nlink_width", "int", &nlink_width, sizeof(int)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:scontext_width", "int", &scontext_width, sizeof(int)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:owner_width", "int", &owner_width, sizeof(int)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:group_width", "int", &group_width, sizeof(int)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:author_width", "int", &author_width, sizeof(int)};
  writeValue(nv, statefile);
  
  nv = (nvpair){"GLOBAL:major_device_number_width", "int", &major_device_number_width, sizeof(int)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:minor_device_number_width", "int", &minor_device_number_width, sizeof(int)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:file_size_width", "int", &file_size_width, sizeof(int)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:format", "enum format", &format, sizeof(enum format)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:time_type", "enum time_type", &time_type, sizeof(enum time_type)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:sort_type", "enum sort_type", &sort_type, sizeof(enum sort_type)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:sort_reverse", "bool", &sort_reverse, sizeof(bool)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:print_owner", "bool", &print_owner, sizeof(bool)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:print_author", "bool", &print_author, sizeof(bool)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:print_group", "bool", &print_group, sizeof(bool)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:numeric_ids", "bool", &numeric_ids, sizeof(bool)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:print_block_size", "bool", &print_block_size, sizeof(bool)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:human_output_opts", "int", &human_output_opts, sizeof(int)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:output_block_size", "uintmax_t", &output_block_size, sizeof(uintmax_t)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:file_human_output_opts", "int", &file_human_output_opts, sizeof(int)};
  writeValue(nv, statefile);
