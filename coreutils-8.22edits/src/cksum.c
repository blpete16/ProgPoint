...
FILE* statefile;
char* statefilename = "/home/brian/Desktop/gradschool/cksum_statefile.dat";
...
void writeState(nvpair* localpairs, int localpaircount, char* statename){
  nvpair nv = {"GLOBAL:have_read_stdin", "bool", &have_read_stdin, sizeof(bool)};
  //static bool have_read_stdin;

  int paircount = 1 + localpaircount;
...
cksum (const char *file, bool print_name)
{
  ...
  char const *hp;
    ...
    locals[0] = (nvpair){"LOCAL:buf", "unsigned char *", buf, sizeof(unsigned char) * BUFLEN};
    locals[1] = (nvpair){"LOCAL:crc", "uint_fast32_t", &crc, sizeof(uint_fast32_t)};
    locals[2] = (nvpair){"LOCAL:length", "uintmax_t", &length, sizeof(uintmax_t)};
    locals[3] = (nvpair){"LOCAL:bytes_read", "size_t", &bytes_read, sizeof(size_t)};
    locals[4] = (nvpair){"LOCAL:length_buf", "char *", length_buf, sizeof(char)*INT_BUFSIZE_BOUND(uintmax_t)};

writeState(locals, 5, "cksum1");
...
writeState(locals, 5, "cksum2");
  fadvise (fp, FADVISE_SEQUENTIAL);

writeState(locals, 5, "cksum3");
  while ((bytes_read = fread (buf, 1, BUFLEN, fp)) > 0)
    {
      unsigned char *cp = buf;

writeState(locals, 5, "cksum1loop1");
...
writeState(locals, 5, "cksum6");
  for (; length; length >>= 8)
    crc = (crc << 8) ^ crctab[((crc >> 24) ^ length) & 0xFF];

  crc = ~crc & 0xFFFFFFFF;

writeState(locals, 5, "cksum7");
  if (print_name)
...
/*BLP - top of main */
    ...
    locals[1] = (nvpair){"LOCAL:ok", "bool", &ok, sizeof(bool)};


/*end top of main */


writeState(locals, 2, "main1");
