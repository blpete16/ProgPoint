...
FILE* statefile;
char* statefilename = "/home/brian/Desktop/gradschool/comm_statefile.dat";
...
void writeState(nvpair* localpairs, int localpaircount, char* statename){
  nvpair nv = {"GLOBAL:hard_LC_COLLATE", "bool", &hard_LC_COLLATE, sizeof(bool)};
...
nv = (nvpair){"GLOBAL:only_file_1", "bool", &only_file_1, sizeof(bool)};
writeValue(nv, statefile);

nv = (nvpair){"GLOBAL:only_file_2", "bool", &only_file_2, sizeof(bool)};
writeValue(nv, statefile);

nv = (nvpair){"GLOBAL:both", "bool", &both, sizeof(bool)};
writeValue(nv, statefile);

nv = (nvpair){"GLOBAL:seen_unpairable", "bool", &seen_unpairable, sizeof(bool)};
writeValue(nv, statefile);

nv = (nvpair){"GLOBAL:issued_disorder_warning", "bool *", issued_disorder_warning, sizeof(bool) * 2};
writeValue(nv, statefile);
...
compare_files (char **infiles)
{
  ...
  int i, j;
    ...
    locals[0] = (nvpair){"LOCAL:lba", "struct linebuffer *", lba, sizeof(struct linebuffer) * 2 * 4};
    locals[1] = (nvpair){"LOCAL:alt", "int *", alt, sizeof(int)*2*3};
    locals[2] = (nvpair){"LOCAL:i", "int", &i, sizeof(int)};
    locals[3] = (nvpair){"LOCAL:j", "int", &j, sizeof(int)};


writeState(locals, 4, "compare_files1");

  /* Initialize the storage. */
  for (i = 0; i < 2; i++)
    {
writeState(locals, 4, "compare_files1loop1");
      for (j = 0; j < 4; j++)
        {

writeState(locals, 4, "compare_files2loop1");
          initbuffer (&lba[i][j]);
...
writeState(locals, 4, "compare_files1loop2");
...
writeState(locals, 4, "compare_files2");
  while (thisline[0] || thisline[1])
    {
      int order;
      bool fill_up[2] = { false, false };

    locals[4] = (nvpair){"LOCAL:order", "int", &order, sizeof(int)};
    locals[5] = (nvpair){"LOCAL:fill_up", "bool *", fill_up, sizeof(bool)*2};
      /* Compare the next available lines of the two files.  */

writeState(locals, 6, "compare_files3loop1");
      if (!thisline[0])
        order = 1;
      else if (!thisline[1])
        order = -1;
...
writeState(locals, 6, "compare_files3loop2");
      for (i = 0; i < 2; i++)
        if (fill_up[i])
          {

writeState(locals, 6, "compare_files4loop1");
...
writeState(locals, 6, "compare_files4loop2");
            if (ferror (streams[i]))
              error (EXIT_FAILURE, errno, "%s", infiles[i]);
...
writeState(locals, 6, "compare_files3");
  for (i = 0; i < 2; i++)
...
/*BLP - top of main */
    ...
    nvpair* locals = malloc(sizeof(nvpair)*1);

    locals[0] = (nvpair){"LOCAL:c", "int", &c, sizeof(int)};


/*end top of main */

writeState(locals, 1, "main1");
...
writeState(locals, 1, "main3");
 while ((c = getopt_long (argc, argv, "123", long_options, NULL)) != -1){

writeState(locals, 1, "main1loop1");
    switch (c)
      {
      case '1':
        only_file_1 = false;
...
writeState(locals, 1, "main5");
  compare_files (argv + optind);

writeState(locals, 1, "main6");
 finishState();
