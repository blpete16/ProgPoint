...
do_encode (FILE *in, FILE *out, uintmax_t wrap_column)
{
  ...
  size_t sum;
    ...
    nvpair* locals = malloc(sizeof(nvpair)*5);

    locals[0] = (nvpair){"LOCAL:current_column", "size_t", &current_column, sizeof(size_t)};
    locals[1] = (nvpair){"LOCAL:inbuf", "char []", &inbuf, sizeof(char)*ENC_BLOCKSIZE};
    locals[2] = (nvpair){"LOCAL:outbuf", "char []", &outbuf, sizeof(char)*ENC_B64BLOCKSIZE};
    locals[3] = (nvpair){"LOCAL:sum", "size_t", &sum, sizeof(size_t)};

  writeState(locals, 4, "doencodeA1");
  do
    {
      size_t n;
      
      locals[4] = (nvpair){"LOCAL:n", "size_t", &n, sizeof(size_t)};

      
  writeState(locals, 5, "doencodeA1loop1.1");

      sum = 0;
      do
        {
	  writeState(locals, 5, "doencodeA1loop2.1");          
  ...
  writeState(locals, 4, "doencodeA2");
...
do_decode (FILE *in, FILE *out, bool ignore_garbage)
{
  ...
  struct base64_decode_context ctx;
    ...
    locals[0] = (nvpair){"LOCAL:inbuf", "char []", &inbuf, sizeof(char)*ENC_BLOCKSIZE};
    locals[1] = (nvpair){"LOCAL:outbuf", "char []", &outbuf, sizeof(char)*ENC_B64BLOCKSIZE};
    locals[2] = (nvpair){"LOCAL:sum", "size_t", &sum, sizeof(size_t)};
    locals[3] = (nvpair){"LOCAL:ctx", "struct base64_decode_context", &ctx, sizeof(struct base64_decode_context)};

  writeState(locals, 4, "doencodeB1");
  ...
  do
    {
      ...
      locals[4] = (nvpair){"LOCAL:ok", "bool", &ok, sizeof(bool)};
      locals[5] = (nvpair){"LOCAL:n", "size_t", &n, sizeof(size_t)};
    locals[6] = (nvpair){"LOCAL:k", "unsigned int", &k, sizeof(unsigned int)};

      sum = 0;
   
  writeState(locals, 7, "doencodeB1LoopA1");
      do
        {
          n = fread (inbuf + sum, 1, DEC_B64BLOCKSIZE - sum, in);

  writeState(locals, 7, "doencodeB1LoopB1");
          if (ignore_garbage)
            {
              size_t i;
              for (i = 0; n > 0 && i < n;)
  ...
  writeState(locals, 4, "doencodeBend");
  free(locals);
}
...
/*BLP - top of main */
    ...
    locals[0] = (nvpair){"LOCAL:opt", "int", &opt, sizeof(int)};
    locals[1] = (nvpair){"LOCAL:input_fh", "FILE *", &input_fh, sizeof(FILE *)};
    locals[2] = (nvpair){"LOCAL:decode", "bool", &decode, sizeof(bool)};
    locals[3] = (nvpair){"LOCAL:ignore_garbage", "bool", &ignore_garbage, sizeof(bool)};
    locals[4] = (nvpair){"LOCAL:wrap_column", "uintmax_t", &wrap_column, sizeof(uintmax_t)};

    
  writeState(locals, 5, "main1");
/*end top of main */
  ...
  textdomain (PACKAGE);
  writeState(locals, 5, "main2");

  atexit (close_stdout);

  while ((opt = getopt_long (argc, argv, "diw:", long_options, NULL)) != -1)
  writeState(locals, 5, "main1loop1");
    switch (opt)
      {
      case 'd':
        decode = true;
