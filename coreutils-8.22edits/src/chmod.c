...
FILE* statefile;
char* statefilename = "/home/brian/Desktop/gradschool/chmodstatefile.dat";
...
void writeState(nvpair* localpairs, int localpaircount, char* statename){
  nvpair nv;
  int globalpaircount = 7;
  int paircount = globalpaircount + localpaircount;
  ...
  if(change != NULL){
    nv = (nvpair){"GLOBAL:*change", "mode_change", change, sizeof(struct mode_change)};
    writeValue(nv, statefile);
  }
  else
  {
    int nothing = 0;
    nv = (nvpair){"GLOBAL:*change", "mode_change", &nothing, sizeof(struct mode_change)};
    writeValue(nv, statefile);
  }
  nv = (nvpair){"GLOBAL:umask_value", "mode_t", &umask_value, sizeof(mode_t)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:recurse", "bool", &recurse, sizeof(bool)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:force_silent", "bool", &force_silent, sizeof(bool)};
  writeValue(nv, statefile);
  
  nv = (nvpair){"GLOBAL:diagnose_surprises", "bool", &diagnose_surprises, sizeof(bool)};
  writeValue(nv, statefile);

  nv = (nvpair){"GLOBAL:verbosity", "Verbosity", &verbosity, sizeof(enum Verbosity)};
  writeValue(nv, statefile);
  
  if(root_dev_ino != NULL){
    nv = (nvpair){"GLOBAL:*root_dev_ino", "dev_ino", root_dev_ino, sizeof(struct dev_ino)};
    writeValue(nv, statefile);
  }
  else
  {
    int nothing = 0;
    nv = (nvpair){"GLOBAL:*root_dev_ino", "dev_ino", &nothing, sizeof(struct dev_ino)};
    writeValue(nv, statefile);
  }
