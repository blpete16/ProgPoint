    ...
    char *iname;
{
  ...
  locals[0] = (nvpair){"iname", "char*", iname, sizeof(char)*strlen(iname)};
  writeState(locals, 1, "treat_file1");
    ...
    writeState(locals, 1, "treat_file2");
...
#  endif
    ...
    writeState(locals, 1, "treat_file3");
    if (!S_ISREG(istat.st_mode)) {
	WARN((stderr,
    ...
    writeState(locals, 1, "treat_file4");
    ifile_size = istat.st_size;
    time_stamp = no_time && !list ? 0 : istat.st_mtime;
    writeState(locals, 1, "treat_file5");
    ...
    writeState(locals, 1, "treat_file6");
    ...
    writeState(locals, 1, "treat_file7");
    clear_bufs(); /* clear input and output buffers */
    part_nb = 0;
    ...
    writeState(locals, 1, "treat_file8");
    ...
    writeState(locals, 1, "treat_file9");
    ...
    writeState(locals, 1, "treat_file10");
    /* Actually do the compression/decompression. Loop over zipped members.
     */
    for (;;) {
      writeState(locals, 1, "treat_file_loop1");
	if ((*work)(ifd, ofd) != OK) {
	    method = -1; /* force cleanup */
	    break;
	}
	writeState(locals, 1, "treat_file_loop2");
	if (!decompress || last_member || inptr == insize) break;
	/* end of file */
	...
	bytes_out = 0;            /* required for length check */
	writeState(locals, 1, "treat_file_loop3");
    }
    ...
    int in;        /* input file descriptor */
{
	...
	/* To simplify the code, we support a zip file when alone only.
         ...
         */
      writeState(NULL,0, "make_ofname1");
        inptr = 0;
	work = unzip;
